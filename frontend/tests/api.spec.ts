...
describe("ApiClient season and inspection requests", () => {
  ...
  it("uses the completed settings contract", async () => {
    const responses = [
      ...
      { enabled: true, ready: true, capabilities: { magnet: true, share: false }, cookie: { source: "file", configured: true, structure_valid: true, sync_status: "success", last_sync_at: null }, target_configured: true, max_concurrency: 1 },
