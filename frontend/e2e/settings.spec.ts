...
test("settings contract, cursor logs, validation states, and responsive layout", async ({ page }, testInfo) => {
  ...
  await page.route("**/api/v1/logs?**", (route) => {
    ...
    if (params.get("category") === "search") return route.fulfill({ json: { items: [{ id: 9, timestamp: "2026-07-25T02:03:00Z", level: "INFO", category: "search", message: "search.new_category", message_zh: "新分类响应" }], next_cursor: null } });
    if (params.get("cursor") === "20") return route.fulfill({ json: { items: [{ id: 2, timestamp: "2026-07-25T02:01:00Z", level: "WARNING", category: "cache", message: "cache.refreshed", message_zh: "缓存已刷新（重复）" }, { id: 3, timestamp: "2026-07-25T02:02:00Z", level: "ERROR", category: "security", message: "auth.relogin_required", message_zh: "需要重新登录" }], next_cursor: null } });
