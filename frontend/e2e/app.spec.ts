...
test("automatically inspects eight magnets and continues without duplicates", async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole("group", { name: "资源质量筛选" })).toBeVisible();
...
test("syncs the inspection setting from the settings page into the live detail workflow", async ({ page }) => {
  ...
  await page.route("**/api/v1/settings/p115", (route) => route.fulfill({ json: { enabled: false, ready: false, capabilities: { magnet: false, share: false }, cookie: { source: "file", configured: false, structure_valid: false, sync_status: "unknown", last_sync_at: null }, target_configured: false, max_concurrency: 1 } }));
