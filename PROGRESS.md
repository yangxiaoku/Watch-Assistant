...
- `REQ-017`：本开发分支补齐离线 SQLite 恢复核心和非敏感配置导出：显式确认和停机声明、
  SHA-256/SQLite/迁移校验、恢复前快照、原子替换、sidecar 清理、失败回滚、脱敏维护脚本、
  `GET /api/v1/backups/configuration` 和 `watchctl backup configuration`；该分支尚未合入发布
  基线。Web 二次批准、在线任务排空证明、恢复后订阅/任务/库存/STRM 校验、配置导入和加密
  异地备份仍待完成。
