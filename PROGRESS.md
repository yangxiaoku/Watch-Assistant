...
当前生产发布基线：`codex/publish-main` @ `4e46e85`（2026-07-30）；systemd release
已部署到 `192.168.6.236:8115`，健康检查通过。
整理计划/执行、移动/重命名、永久删除和 STRM 播放已按真实受管夹具/直链证据开启；
STRM 全量、增量和清理因测试目录没有视频文件，继续保持关闭。

## 已发布

- 115 真实操作门禁：整理计划/执行、移动/重命名写入、永久删除和 STRM 播放已部署并开启；
  固定测试 CID 的组织闭环成功，永久删除单文件后目录与回收站均为空，播放 `HEAD=200`、
  单段 `Range GET=206`。STRM 生成/清理暂不启用，等待受管视频夹具。
...
- `REQ-002`：需要受管视频夹具完成 STRM 全量/增量/清理验收；当前三个 STRM 写入开关保持关闭。
