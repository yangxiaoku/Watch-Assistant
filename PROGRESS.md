...
- `REQ-017`：本开发分支补齐离线 SQLite 恢复核心：显式确认和停机声明、SHA-256/SQLite/迁移
  校验、恢复前快照、原子替换、sidecar 清理、失败回滚、脱敏维护脚本及恢复事件；组合回归
  187 项通过，其中备份/恢复专项 16 项。该分支尚未合入发布基线；Web 二次批准、
  在线任务排空证明、恢复后订阅/任务/库存/STRM 校验和配置导入导出仍待完成。
