...
## 2026-08-16 第七轮 E2E:安全边界与禁用态验证(已合入基线)

- 新增第七轮 E2E（`frontend/e2e/live/module-flows6.spec.ts`）5 项用户操作：推送按钮在
  能力不可用时禁用并带原因提示（磁力云下载不可用/115 分享转存尚未验证）、整理工作台
  执行按钮在无计划时不可触发（门禁呈现）、115 自动签到分区开关与时间控件只读断言、
  离线横幅断网提示与恢复；连同既有用例共 63/63 通过（1 容错跳过）。本轮无产品缺陷。
- 七轮累计：真机 E2E 63 项覆盖全部安全可行用户操作面；累计修复 3 个产品缺陷
  （取消按钮死按钮/已取消订阅无法重新订阅/SPA 回退缺失）。受 115 风控与安全门禁
...
## 2026-08-16 Prowlarr「总是失效」排查与修复(已上线)

- 症状定位:设置页显示「服务端配置暂时熔断」;根因为服务器出网受限——启用索引器
  中 Nyaa/YTS/Mikan/Bangumi Moe/Bitsearch/LimeTorrents 全部连接超时(宿主机与容器
  内均 000),仅 1377x 等极少数可达;Prowlarr 搜索/验证连续失败 3 次触发熔断,
  半开探测又失败形成循环,用户感知「总是失效」。
- 修复:连接验证从 45s 搜索预算缩短为 15s 短超时(验证只确认连通性),失败仍计入
  健康状态机、半开探测自动恢复,契约不变;发布 `80838a7`(verify.sh 通过)已部署,
  `POSTDEPLOY_RELEASE_CHECK=ok`,全量 E2E 回归通过。
- 结论:代码行为符合熔断设计;持续失效的根源是环境出网受限。建议:在 Prowlarr 中
  禁用不可达索引器(保留 1377x 等可达源),或关闭 Prowlarr 来源(panSou 主源正常,
  E2E 搜索「来源完整」);此项为运维配置决策,未擅自修改用户 Prowlarr 配置。
