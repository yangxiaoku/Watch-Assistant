...
- `REQ-009`：整理 operation 已支持通过受保护 API 关联 workflow，并在排队、claim、成功、
  ...
  `failed` 的问题，并补齐任务 ID、115 不可用、订阅新资源和备份失败通知；本轮新增分页
  workflow 子任务 API、`watchctl workflow children` 和任务中心详情展示，聚合推送、检测、
  整理及 STRM 阶段子任务，并区分子任务原始状态与 workflow 阶段状态。通知完整事件矩阵、
  可靠渠道和生产跨系统验收仍待完成。
- `REQ-012/REQ-019`：Webhook 已提供管理员保护的单端点测试通知入口，测试事件只进入该端点
  ...
  真实接收端和完整事件矩阵仍待验收；本轮补齐内容检测失败、自动整理阻断/失败和 STRM
  校验异常的站内通知策略，成功类普通日志保持静默。
