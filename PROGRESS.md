...
- 当前本地工作区：`/Users/apple/Desktop/115ts`（macOS）。旧外置卷工作区不再使用。
- 当前检出的开发分支不等于生产发布；本地工作只从最新 `origin/codex/publish-main`
  创建独立功能分支，合并前不得描述为已发布。
- 当前工作区尚未包含 `.venv`、Docker CLI 或 PowerShell；验证前需准备当前工作区的
  `.venv/bin/python`，不得使用系统 Python 或另一工作树的已安装包替代。
