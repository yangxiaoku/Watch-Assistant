...
- `REQ-012/REQ-019`：Webhook 已提供管理员保护的单端点测试通知入口，测试事件只进入该端点
  ...
  真实接收端和完整事件矩阵仍待验收。
- `REQ-003`：`watchctl library scan <library-id>` 已接入正式媒体库扫描 API，支持显式或自动生成的幂等键并统一 JSON envelope；本次仅请求服务端根目录完整扫描，不开放路径级扫描或 dry-run。Web 高风险批准、完整任务关联、生产播放入口和跨平台端到端验收仍未完成，生产 STRM 继续门控。
