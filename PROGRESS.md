...
- REQ-018：新增只读健康报告 API 和幂等报告账本，报告从库存扫描、受管 STRM 清单和目标目录任务构造；
  严重/错误问题通过 library.health.critical 接入站内通知，不完整库存依赖问题不会误报。生产库存、
  直链/字幕/元数据检查、定时/复检 worker 和完整通知矩阵仍待完成；该切片尚未合入发布基线。
