...
整理计划/执行、移动/重命名、永久删除和 STRM 全量/增量/清理/播放已按真实受管夹具
证据开启。

## 已发布

- 115 真实操作门禁：整理计划/执行、移动/重命名写入、永久删除和 STRM 全量/增量/清理/
  播放已部署并开启。固定测试 CID 的组织闭环成功，视频夹具完成全量生成、改名后的增量
  对账、删除后的清理和永久删除后置核验；播放 `HEAD=200`、单段 `Range GET=206`。
...
- `REQ-002`：受管视频夹具的 STRM 全量/增量/清理验收已完成；后续仍需生产影视库首次
  扫描和用户目录级回归。
