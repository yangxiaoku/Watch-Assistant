...
- 失败后系统**无退避**地反复重试：10:39:56-10:40:51 出现 26 次 `inventory.refresh.failed`（约每 2.1 秒 1 次）——30 个订阅任务逐个触发「守卫→全库扫描」，每个扫描对 2 库 × 2 端点发起请求，两分钟内约 120 次 115 请求，持续喂养风控窗口；
- 网关对 405 的退避只有 45s × 2 次（`_GATEWAY_CALL_405_RETRY_DELAY_SECONDS`），窗口持续数小时时形同虚设；
- 自动整理调度（120min）+ 整理触发（日志约每 35-45 分钟一次 blocked 尝试）继续打 115。
