...
更新时间：2026-07-30  
审计基线：`codex/publish-main@0a4a2e2`

本文件是 2026-07-30 的历史审计记录；其中的提交、开关和运行环境只描述当时的证据，
不代表当前 `codex/publish-main`、当前集成分支或生产部署状态。
...
| ORG-002 扫描与变化检测 | 部分证明 | `LibraryIndexService`、断点和完整性门禁、`OrganizationScheduler` 按配置源 CID 扫描；生产根目录最新只读扫描完整 | 生产源目录尚未通过 Web 设置完成自动计划验收；仍需先预览再开启自动领取 |
...
| ORG-010 冲突与计划预览 | 部分证明 | 不可变 plan hash、revision、过期和前置条件；live 组织 runner 完成计划预览 | 生产源/目标配置下的自动计划与冲突样本验收 |
| ORG-011 移动/重命名/伴随联动 | 已证明（受管夹具） | `LiveP115OrganizationTransport`；单文件真实移动/重命名/恢复；6 次回执后核对；受管夹具写 worker 已具备 | 生产媒体库业务计划、伴随文件联动和多对象失败恢复仍需专用计划验收；该证据不授权生产写入 |
...
| Must 项 | 当前状态 | 证据 | 尚缺内容 |
|---|---|---|---|
| STRM-001/002 映射与受管清单 | 部分证明 | `StrmManifestEntry`、唯一 current 约束；live 夹具正式 API 全量生成 | 生产媒体库接线和 pickcode 生命周期字段尚未验收 |
| STRM-003 全量/断点续跑 | 部分证明 | 完整扫描门禁、manifest service 离线测试；live 夹具全量扫描成功 | 取消续跑和 10k fixture 证据 |
| STRM-004 原子生成 | 已证明（受管夹具） | 根目录 allowlist、临时文件、fsync/replace；live 夹具首次生成成功 | 生产输出目录实际运行验收 |
...
| STRM-008 失效/空目录清理 | 部分证明 | 只处理 manifest、完整扫描前置条件；live 夹具恢复后清理成功 | 生产清理仍关闭；非受管文件保护的生产回归 |
...
以上为旧批次证据；当时不授权生产影视库自动整理、STRM 播放、STRM 清理或永久删除。
当前开关和本轮补充证据以同目录的 `req-001-002-live-audit-20260730.md` 为准。

## 历史记录：2026-07-30 线上安全收敛（02a359e）
...
在上述门禁完成前，发布说明只能称为“设置和受控计划/执行基础”，不能称为 REQ-001 或 REQ-002 已完成；历史发布记录也不得被当作当前生产状态。
