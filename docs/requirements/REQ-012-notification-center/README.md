...
- 通知渠道设置页新增“测试”按钮：后端 `POST /api/v1/notify-channels/{id}/test` 会按渠道真实适配器发送一条固定中文测试通知并返回 `ok`；
  渠道不存在/凭据解密失败/CLI 配置无效返回稳定错误码，发送失败不抛错并记录 `notify.delivery_failed`，成功记录 `notify.test_delivered`。
