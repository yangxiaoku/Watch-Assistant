...
> **SHA 时效说明（2026-08-11）**：下方第 66 行的 `5df17fb` / `249aa582` 是 **2026-08-05 的
> 历史发布线/部署快照**，仅作当时状态记录；当前发布线一律以
> `git rev-parse origin/codex/publish-main` 动态解析为准，勿按硬编码值断言当前版本。
...
2026-08-05 当前发布基线只读核对：动态解析的 `origin/codex/publish-main` 完整 SHA 为 `5df17fb003ae8ea3a4774592d1fc81cb4aa78ead`（短标识 `5df17fb`）。公开 [Verify run](https://github.com/yangxiaoku/Watch-Assistant/actions/runs/30984240700) 和当前 ref 的 Systemd Release Package 均成功，产物为 `watch-assistant-5df17fb-20260805-0719.tar.gz`。服务器 systemd 当前仍运行 `249aa5826db6e06683be6426aa5243a1aa4ed0f5`，健康接口返回 `status=ok`，因此当前发布线尚未部署。服务器部署版本的受限生产只读库存为 54 页、12 个目录、43 个文件、54 个唯一对象，整理预览为 1 个 `needs_review`、远程写入 0；C03 因一次性授权已消费而 blocked。受管视频夹具的历史/受限记录曾有 STRM 全量生成 40 个并 verify 40/40，但生产 STRM full 终态为 `failed`（`generated=0`、`failed=40`、verify `checked=40`、`valid=0`、`invalid=40`，输出 `.strm` 文件数为 0）。媒体服务器真实播放兼容、STRM/空目录清理和整理写入继续关闭。
