...
_CATALOG: dict[str, ApiErrorDescriptor] = {
    ...
    "backup_delete_confirmation_required": ApiErrorDescriptor("backup_delete_confirmation_required", "需要确认删除备份", "备份没有删除。", "请明确确认后再删除旧备份。", False),
    "backup_delete_last": ApiErrorDescriptor("backup_delete_last", "不能删除唯一备份", "备份没有删除。", "请先创建另一份可用备份后再清理旧备份。", False),
    "backup_delete_failed": ApiErrorDescriptor("backup_delete_failed", "备份删除失败", "备份状态未确认。", "请检查备份目录后再试，不要重复删除同一备份。", False),
    "encrypted_backup_confirmation_required": ApiErrorDescriptor("encrypted_backup_confirmation_required", "需要确认加密备份", "没有创建加密备份副本。", "请明确确认后再创建加密副本。", False),
    "encrypted_backup_disabled": ApiErrorDescriptor("encrypted_backup_disabled", "加密备份未启用", "没有创建加密备份副本。", "请先配置受控目标目录并显式启用加密备份。", False),
    "encrypted_backup_key_invalid": ApiErrorDescriptor("encrypted_backup_key_invalid", "恢复密钥无效", "加密备份操作没有完成。", "请使用独立的 Fernet 恢复密钥并通过安全输入提供。", False),
    "encrypted_backup_invalid": ApiErrorDescriptor("encrypted_backup_invalid", "加密备份副本无效", "加密备份操作没有完成。", "请检查副本清单和摘要，不要继续恢复损坏副本。", False),
    "encrypted_backup_failed": ApiErrorDescriptor("encrypted_backup_failed", "加密备份副本创建失败", "没有创建完整加密副本。", "请检查受控目标目录和磁盘空间后重试。", True, "retry"),
    ...
    "backup_configuration_confirmation_required": ApiErrorDescriptor("backup_configuration_confirmation_required", "需要确认配置导入", "本次配置没有写入。", "请明确确认后再导入脱敏配置。", False, "inspect_configuration"),
    "backup_configuration_conflict": ApiErrorDescriptor("backup_configuration_conflict", "配置版本已变化", "本次配置没有写入，避免覆盖更新后的设置。", "请重新导出当前配置并再次确认。", False, "reload_settings"),
    "backup_configuration_invalid": ApiErrorDescriptor("backup_configuration_invalid", "配置文件无效", "本次配置没有写入。", "请使用 Watch Assistant 导出的脱敏配置文件。", False, "inspect_configuration"),
    ...
    "restore_requires_service_stop": ApiErrorDescriptor("restore_requires_service_stop", "恢复前需要停止服务", "数据库没有被覆盖。", "请停止服务并确认没有任务或工作进程在运行后再恢复。", False),
    "restore_target_invalid": ApiErrorDescriptor("restore_target_invalid", "恢复目标无效", "数据库没有被覆盖。", "请使用现有的应用数据库文件作为恢复目标。", False),
    "backup_digest_mismatch": ApiErrorDescriptor("backup_digest_mismatch", "备份摘要不匹配", "该备份没有通过完整性校验。", "请使用其他备份，不要继续覆盖数据库。", False),
    "backup_integrity_failed": ApiErrorDescriptor("backup_integrity_failed", "备份数据库损坏", "该备份没有通过 SQLite 完整性校验。", "请使用其他备份，不要继续覆盖数据库。", False),
    "restore_validation_failed": ApiErrorDescriptor("restore_validation_failed", "恢复校验未通过", "数据库没有被确认恢复。", "请检查备份版本和迁移状态后再试。", False),
    "restore_consistency_failed": ApiErrorDescriptor("restore_consistency_failed", "恢复后业务校验未通过", "数据库没有被确认恢复。", "请检查任务、订阅、库存和 STRM 清单的一致性后再试。", False),
    "restore_failed": ApiErrorDescriptor("restore_failed", "数据库恢复失败", "当前数据库需要继续核对，恢复结果未确认。", "请保留恢复前快照并检查磁盘和数据库状态。", False),
    "restore_rollback_failed": ApiErrorDescriptor("restore_rollback_failed", "恢复回滚失败", "数据库恢复和回滚都未完成，当前状态需要人工核对。", "不要重启或重复恢复，请先保留备份文件并人工检查数据库。", False),
    "restore_approval_confirmation_required": ApiErrorDescriptor("restore_approval_confirmation_required", "需要确认恢复批准", "本次恢复批准未创建。", "请在预览通过后明确确认恢复批准。", False),
    "restore_approval_unavailable": ApiErrorDescriptor("restore_approval_unavailable", "恢复批准服务暂不可用", "本次恢复批准未创建。", "请稍后重试。", True, "retry"),
    "restore_preview_not_ready": ApiErrorDescriptor("restore_preview_not_ready", "恢复预览未通过", "本次恢复批准未创建。", "请修复备份校验或迁移兼容性问题后重试。", False),
    "restore_approval_not_found": ApiErrorDescriptor("restore_approval_not_found", "恢复批准不存在", "本次恢复批准未完成。", "请刷新恢复记录后再试。", False),
    "restore_approval_expired": ApiErrorDescriptor("restore_approval_expired", "恢复批准已过期", "本次恢复没有执行。", "请重新生成恢复预览和批准申请。", False),
    "restore_second_approver_required": ApiErrorDescriptor("restore_second_approver_required", "需要另一位批准人", "本次恢复没有执行。", "请使用不同的管理员会话完成二次批准。", False),
    "restore_preview_changed": ApiErrorDescriptor("restore_preview_changed", "恢复预览已变化", "本次恢复没有执行。", "请重新生成恢复批准并再次核对备份。", False),
    "restore_drain_incomplete": ApiErrorDescriptor("restore_drain_incomplete", "后台任务尚未排空", "本次恢复没有执行。", "请取消或完成未结束任务后重新查看安全点。", False),
    "maintenance_mode_required": ApiErrorDescriptor("maintenance_mode_required", "尚未进入维护模式", "本次恢复没有执行。", "请先进入在线维护模式。", False),
    "restore_approval_conflict": ApiErrorDescriptor("restore_approval_conflict", "恢复批准状态已变化", "本次恢复没有执行。", "请刷新恢复批准记录。", False),
    "restore_approval_required": ApiErrorDescriptor("restore_approval_required", "需要恢复批准", "应用数据库没有被覆盖。", "请先通过 Web 完成二次批准，并在离线脚本中提供批准编号。", False),
    "restore_approval_invalid": ApiErrorDescriptor("restore_approval_invalid", "恢复批准无效", "应用数据库没有被覆盖。", "请重新生成恢复预览和二次批准，不要重复使用旧批准。", False),
    "maintenance_mode_active": ApiErrorDescriptor("maintenance_mode_active", "系统正在维护", "本次新任务没有创建。", "请等待维护完成后再试。", True, "retry"),
    "maintenance_unavailable": ApiErrorDescriptor("maintenance_unavailable", "维护模式暂不可用", "本次维护操作未完成。", "请稍后重试。", True, "retry"),
