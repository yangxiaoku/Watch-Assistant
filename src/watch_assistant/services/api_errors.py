...
_CATALOG: dict[str, ApiErrorDescriptor] = {
    ...
    "task_not_reconcilable": ApiErrorDescriptor("task_not_reconcilable", "当前任务没有可核对的远端标识", "本次只读核对未执行，任务状态没有改变。", "请查看任务状态；只有已取得远端标识的任务才能核对。", False, "view_task"),
    "reconciliation_unavailable": ApiErrorDescriptor("reconciliation_unavailable", "远端核对暂时不可用", "本次只读核对未完成，任务状态没有被伪造为成功。", "请稍后重新核对；在核对完成前不要重复提交。", True, "retry"),
    "reconciliation_conflict": ApiErrorDescriptor("reconciliation_conflict", "任务状态已发生变化", "本次只读核对结果没有写入，系统保留最新任务状态。", "请刷新任务后重新核对。", True, "refresh_snapshot"),
    ...
    "library_scan_unavailable": ApiErrorDescriptor("library_scan_unavailable", "媒体库扫描服务暂不可用", "本次扫描没有排队，远端目录没有被修改。", "请检查 115 连接后重试。", True, "retry"),
    "library_scan_not_found": ApiErrorDescriptor("library_scan_not_found", "扫描操作不存在", "本次扫描状态无法读取。", "请刷新媒体库扫描记录后再试。", False, "refresh_snapshot"),
    "invalid_library_id": ApiErrorDescriptor("invalid_library_id", "媒体库标识无效", "本次扫描请求未执行。", "请从媒体库列表选择有效的媒体库。", False),
    "invalid_scan_run_id": ApiErrorDescriptor("invalid_scan_run_id", "扫描操作标识无效", "本次扫描请求未执行。", "请从扫描记录重新选择操作。", False),
    "invalid_directory_limit": ApiErrorDescriptor("invalid_directory_limit", "扫描目录上限无效", "本次扫描没有排队。", "请使用有效的目录数量上限。", False),
    "scan_mode_unsupported": ApiErrorDescriptor("scan_mode_unsupported", "扫描模式不受支持", "本次扫描没有执行。", "请重新发起媒体库扫描。", False),
    "checkpoint_invalid": ApiErrorDescriptor("checkpoint_invalid", "扫描断点无效", "本次扫描没有继续执行，已避免重复读取结果。", "请重新发起媒体库扫描。", False, "refresh_snapshot"),
    "plan_hash_mismatch": ApiErrorDescriptor("plan_hash_mismatch", "计划摘要已变化", "本次确认未执行，计划内容没有被静默修改。", "请刷新计划并重新确认。", False, "reload_settings"),
    "empty_directory_cleanup_disabled": ApiErrorDescriptor("empty_directory_cleanup_disabled", "空目录清理未启用", "本次空目录清理计划未生成。", "请先在整理设置中启用受管空目录清理。", False, "inspect_configuration"),
    "empty_directory_cleanup_unavailable": ApiErrorDescriptor("empty_directory_cleanup_unavailable", "空目录清理服务暂时不可用", "本次空目录清理没有执行，远端状态没有被伪造更新。", "请检查 115 连接和写入契约后重试。", True, "retry"),
    "empty_cleanup_expired": ApiErrorDescriptor("empty_cleanup_expired", "空目录清理计划已过期", "本次空目录清理未执行。", "请重新扫描并生成清理计划。", False, "refresh_snapshot"),
    "empty_cleanup_changed": ApiErrorDescriptor("empty_cleanup_changed", "空目录清理候选已变化", "本次空目录清理未执行，避免误回收目录。", "请重新扫描并生成清理计划。", False, "refresh_snapshot"),
    ...
    "organization_history_unavailable": ApiErrorDescriptor("organization_history_unavailable", "整理历史服务暂时不可用", "整理历史没有更新。", "请稍后重试。", True, "retry"),
    "invalid_source_object": ApiErrorDescriptor("invalid_source_object", "来源影片标识无效", "本次影片选择未执行。", "请从待确认列表重新选择。", False),
    "invalid_tmdb_candidate": ApiErrorDescriptor("invalid_tmdb_candidate", "TMDB 候选无效", "本次影片选择未执行。", "请刷新候选列表后重试。", False),
    "tmdb_candidate_not_found": ApiErrorDescriptor("tmdb_candidate_not_found", "TMDB 候选不存在", "本次影片选择未执行。", "请刷新整理计划后重试。", False, "retry"),
    "candidate_target_unavailable": ApiErrorDescriptor("candidate_target_unavailable", "候选归档路径不可用", "本次影片没有开始整理。", "请检查归档目录和分类设置。", False),
    "source_snapshot_mismatch": ApiErrorDescriptor("source_snapshot_mismatch", "来源扫描快照已变化", "本次影片没有开始整理。", "请重新扫描后再选择影片。", False, "reload_settings"),
    ...
    "contract_unverified": ApiErrorDescriptor("contract_unverified", "115 整理写入契约尚未验收", "本次 115 写入操作未执行。", "请先完成受管测试目录的写入契约验收。", False, "inspect_configuration"),
    "capability_unverified": ApiErrorDescriptor("capability_unverified", "115 整理能力尚未完成契约验收", "本次 115 写入操作未执行。", "请先完成对应移动或重命名能力的契约验收。", False, "inspect_configuration"),
    "organization_lease_required": ApiErrorDescriptor("organization_lease_required", "整理操作租约无效", "本次目标目录没有创建。", "请从已确认的整理操作重新执行，系统不会在预览阶段创建目录。", False, "refresh_snapshot"),
    "target_directory_parent_missing": ApiErrorDescriptor("target_directory_parent_missing", "归档目录父级不存在", "本次目标目录没有创建。", "请确认整理计划和目标目录范围后重新生成计划。", False, "refresh_snapshot"),
    "target_directory_create_failed": ApiErrorDescriptor("target_directory_create_failed", "归档目录创建失败", "本次目标目录没有完成创建。", "请先核对 115 远端目录状态，再决定是否重新执行。", False, "view_task"),
    ...
    "strm_library_operation_conflict": ApiErrorDescriptor("strm_library_operation_conflict", "STRM 媒体库操作冲突", "本次 STRM 操作未开始，避免并发修改媒体库清单。", "请等待当前操作完成后再重试。", True, "retry"),
    "strm_operation_in_progress": ApiErrorDescriptor("strm_operation_in_progress", "STRM 操作正在执行", "本次请求未重复执行。", "请稍后刷新操作状态。", True, "retry"),
    "strm_operation_lease_lost": ApiErrorDescriptor("strm_operation_lease_lost", "STRM 操作执行权已变化", "旧执行已停止继续更新文件或状态。", "请查看操作状态，确认结果后再决定是否重试。", False, "view_task"),
    "cleanup_plan_required": ApiErrorDescriptor("cleanup_plan_required", "需要先确认 STRM 清理计划", "本次清理没有执行任何退休操作。", "请打开清理预览，核对候选后再确认执行。", False, "refresh_snapshot"),
    ...
    "organization_reconciliation_unavailable": ApiErrorDescriptor("organization_reconciliation_unavailable", "整理结果核对服务暂不可用", "本次远端只读核对未完成。", "请稍后重试，系统不会自动重复写入。", True, "retry"),
    ...
    "invalid_source_directory_labels": ApiErrorDescriptor("invalid_source_directory_labels", "源目录显示名称无效", "本次整理设置未保存。", "请重新从目录选择器选择安全的名称或相对路径。", False),
    "invalid_target_directory_id": ApiErrorDescriptor("invalid_target_directory_id", "目标目录 ID 无效", "本次整理设置未保存。", "请填写纯数字的 115 目录 CID。", False),
    "invalid_target_directory_label": ApiErrorDescriptor("invalid_target_directory_label", "归档目录显示名称无效", "本次整理设置未保存。", "请重新从目录选择器选择安全的名称或相对路径。", False),
    "invalid_push_directory_id": ApiErrorDescriptor("invalid_push_directory_id", "推送目录 ID 无效", "本次整理设置未保存。", "请从 115 目录选择器选择推送目录。", False),
    "invalid_push_directory_label": ApiErrorDescriptor("invalid_push_directory_label", "推送目录显示名称无效", "本次整理设置未保存。", "请重新从目录选择器选择安全的名称或相对路径。", False),
    ...
    "cleanup_plan_already_applied": ApiErrorDescriptor("cleanup_plan_already_applied", "STRM 清理计划已执行", "本次请求未重复退休 STRM 文件。", "请刷新清理计划查看已完成结果。", False, "refresh_snapshot"),
    "strm_operation_cancelled": ApiErrorDescriptor("strm_operation_cancelled", "STRM 操作已取消", "本次 STRM 操作已停止，未留下运行中状态。", "请重新扫描后再发起同步。", False, "refresh_snapshot"),
    "strm_operation_recovered": ApiErrorDescriptor("strm_operation_recovered", "STRM 操作已恢复收口", "上次进程中断留下的 STRM 操作已标记为失败。", "请确认扫描快照后重新发起同步。", False, "refresh_snapshot"),
    "strm_operation_timeout": ApiErrorDescriptor("strm_operation_timeout", "STRM 操作已超时", "本次 STRM 操作没有在限定时间内完成。", "请查看操作历史并确认文件状态后再重试。", False, "view_task"),
    ...
    "workflow_prerequisite_not_met": ApiErrorDescriptor("workflow_prerequisite_not_met", "工作流前置阶段未完成", "本次阶段没有开始，系统保持阶段顺序。", "请先完成前置阶段后再继续。", False, "view_task"),
    "workflow_stage_regression": ApiErrorDescriptor("workflow_stage_regression", "工作流阶段不能回退", "本次阶段更新未保存，已有结果保持不变。", "请刷新任务中心查看当前阶段。", False, "view_task"),
    "workflow_stage_terminal": ApiErrorDescriptor("workflow_stage_terminal", "工作流阶段已结束", "本次阶段更新未保存，系统不会改写已结束结果。", "请查看诊断信息并核对关联任务。", False, "view_task"),
    "workflow_evidence_required": ApiErrorDescriptor("workflow_evidence_required", "工作流阶段需要受信证据", "本次阶段更新未保存，不能通过通用接口伪造发现或文件可用结果。", "请让资源发现或只读核对生产对应证据后再继续。", False, "view_task"),
    "workflow_discovery_conflict": ApiErrorDescriptor("workflow_discovery_conflict", "发现证据与工作流不一致", "本次发现结果未覆盖已有工作流状态。", "请刷新工作流并选择正确的资源。", False, "refresh_snapshot"),
