...
class StrmCleanupPlan(Base):
    """A reviewable, non-destructive plan for retiring managed STRM files."""

    __tablename__ = "strm_cleanup_plans"
...
class EmptyDirectoryCleanupPlan(Base):
    """A reviewed, reversible plan for recycling managed empty directories."""

    __tablename__ = "empty_directory_cleanup_plans"
    ...
    applied_deleted: Mapped[int] = mapped_column(Integer, default=0, server_default="0")
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=_utc_now
    )
    updated_at: Mapped[datetime] = mapped_column(
