...
async def _execute_and_restore(
    ...
    *,
    ...
    transport = create_live_p115_organization_transport(
        ...
        intents=(intent,),
        managed_directory_ids=(parent_id,),
        scope_confirmed=True,
        live_enabled=True,
    )
    try:
        ...
        move = await transport.move(intent.object_id, parent_id)
        ...
        if moved != RemoteObjectState(intent.object_id, parent_id, original_name):
        ...
        if renamed != RemoteObjectState(intent.object_id, parent_id, intent.target_name):
    ...
    restore_intent = OrganizationObjectIntent(
        intent.object_id, parent_id, intent.target_name, parent_id, original_name
    )
    ...
    try:
        ...
        if restored_source != RemoteObjectState(intent.object_id, parent_id, intent.target_name) or restored_target is not None:
...
def _uncertain(code: str, *transports) -> dict[str, object]:
    return {
        ...
        "receipt_count": sum(len(transport.receipts) for transport in transports),
        "write_started": True,
    }
