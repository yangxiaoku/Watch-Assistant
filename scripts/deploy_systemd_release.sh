...
if [[ "$COMMAND" == "rollback" ]]; then
    ...
    "$PYTHON_BIN" "$TOOL_ROOT/scripts/postdeploy_release_check.py" \
        --version-file "$CURRENT_ROOT/VERSION" \
...
if ! "$PYTHON_BIN" "$RELEASE_ROOT/scripts/postdeploy_release_check.py" \
    --version-file "$RELEASE_ROOT/VERSION" \
    --expected-release "$EXPECTED_RELEASE" \
    --health-url "$HEALTH_URL" \
    --release-env "$RELEASE_ENV" \
    --current-root "$CURRENT_ROOT" \
    --stale-drop-in "$DROP_IN" \
    --diagnostics-url "$DIAGNOSTICS_URL"; then
