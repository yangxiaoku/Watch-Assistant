...
- 当前环境没有 `node`、`npm`、`pwsh` 或 `docker`。前端 Vitest、构建、Playwright 和
  Docker Compose 检查不能在本机宣称已通过；需在具备对应工具的环境中执行并记录真实结果。
  Windows PowerShell 备份命令只适用于对应 Windows 主机，不作为 macOS 本地命令执行。
...
# Docker Compose：仅在 docker 已安装的环境执行
docker compose config
```
